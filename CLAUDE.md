...
full `void-initfs` summary). This file only adds Claude-Code-specific process notes for working
...
- Subject-line prefixes used: `claude:` (fully Claude-authored change, e.g. the README),
  ...
  `void-initfs.md`). Plain, unprefixed subjects also appear (e.g. `bootstrap: add packages...`,
...
Originally observed over several planning turns on `void-initfs.md` (a hand-edited planning doc
the user kept at the repo root while `void-initfs` was being designed and built); that specific
...
- Small pending-work items get tracked in a lightweight, disposable `TODO.md` at the repo root
  ...
  Files like this (and the old `void-initfs.md`) can be hand-edited by the user between
...
- Work on this repo proceeds incrementally, one reviewed step at a time (comprehensive logging,
  the SSH `authorized_keys` marker fix, the `void-initfs` pipeline built up step by step, the
...
- Build artifacts (`void-init`, `void-initfs`, under `build/` via `Makefile` or at the repo root
  via `go build ./...`) are gitignored (`.gitignore`'s `/build`, `/void-init`, `/void-initfs`
